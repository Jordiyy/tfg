...
for row in datos:
    ...
    score = (age / 365 / 80) * (((ldl / colt ) - 0.3) * (systolic_blood_pressure / 50) * (1.5 * smoke))
    row['score'] = round(math.exp(score), 2)


with open('score_low_col.csv', mode='w', newline='') as file:
...
print("Cálculo completado y guardado en 'score_low_col.csv'")